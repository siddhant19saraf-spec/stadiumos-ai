...
import { Play, Square, Sparkles, CloudRain, Star, Trophy, Zap, ShoppingBag, AlertTriangle, Sun, LogOut, Users, XCircle } from "lucide-react";
...
const scenarioIcons: Record<string, React.ElementType> = {
  halftime_rush: Zap, rain_delay: CloudRain, vip_event: Star, sold_out_match: Trophy,
  counter_failure: XCircle, staff_shortage: Users, emergency_evacuation: AlertTriangle,
  merchandise_drop: ShoppingBag, heat_wave: Sun, post_game_exit: LogOut,
};
    ...
    <Card className={cn("", className)}>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center justify-between text-sm">
          ...
          {active && <Badge variant="outline" className="bg-emerald-500/10 text-[10px] text-emerald-400">ACTIVE</Badge>}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-2 max-h-[400px] overflow-y-auto">
        {(Object.entries(SCENARIO_CONFIGS) as [SimulationScenario, typeof SCENARIO_CONFIGS[SimulationScenario]][]).map(([id, config]) => {
          ...
          return (
            <div key={id} className={cn("rounded-md border transition-all", isRunning && "border-emerald-500/30 bg-emerald-500/5", !isRunning && !active && "hover:border-muted-foreground/20")}>
              <button
              ...
              >
                ...
                <Button variant={isRunning ? "destructive" : "outline"} size="icon" className="h-6 w-6 shrink-0"
