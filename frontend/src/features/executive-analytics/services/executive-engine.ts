import type { ExecutiveSummary, ModuleSnapshot, ExecutiveRole, KpiCategory } from "../types";
...
export interface IExecutiveEngine {
  ...
  getKpiValues(role: ExecutiveRole): { id: string; label: string; value: number; previousValue: number; unit: string; category: KpiCategory; trend: "up" | "down" | "stable"; status: "critical" | "warning" | "healthy" | "neutral"; changePct: number; changeDirection: "increase" | "decrease" | "unchanged"; tooltip: string }[];
}
...
export class MockExecutiveEngine implements IExecutiveEngine {
  ...
  getKpiValues(role: ExecutiveRole) {
    const all: Array<{
      id: string; label: string; value: number; category: KpiCategory; unit: string; tooltip: string;
    }> = [
      { id: "op-health", label: "Operational Health", value: this.operationalHealth(), category: "operations" as KpiCategory, unit: "%", tooltip: "Overall operational performance across all modules" },
      { id: "safety-score", label: "Safety Score", value: this.safetyScore(), category: "safety" as KpiCategory, unit: "%", tooltip: "Safety and security compliance rating" },
      { id: "crowd-health", label: "Crowd Health", value: this.crowdHealth(), category: "crowd" as KpiCategory, unit: "%", tooltip: "Crowd density and flow efficiency" },
      { id: "tournament-progress", label: "Tournament Progress", value: ri(45, 95), category: "tournament" as KpiCategory, unit: "%", tooltip: "Progress against tournament schedule" },
      { id: "infra-health", label: "Infrastructure Health", value: this.infraHealth(), category: "infrastructure" as KpiCategory, unit: "%", tooltip: "Facility and equipment health score" },
      { id: "parking-util", label: "Parking Utilization", value: ri(40, 95), category: "parking" as KpiCategory, unit: "%", tooltip: "Current parking capacity utilization" },
      { id: "queue-perf", label: "Queue Performance", value: ri(55, 92), category: "queue" as KpiCategory, unit: "%", tooltip: "Queue wait time efficiency" },
      { id: "energy-eff", label: "Energy Efficiency", value: this.energyEfficiency(), category: "energy" as KpiCategory, unit: "%", tooltip: "Energy consumption efficiency rating" },
      { id: "carbon-score", label: "Carbon Score", value: this.carbonScore(), category: "sustainability" as KpiCategory, unit: "%", tooltip: "Carbon emission and sustainability score" },
      { id: "financial-perf", label: "Financial Performance", value: this.financialPerformance(), category: "financial" as KpiCategory, unit: "%", tooltip: "Revenue and cost performance index" },
      { id: "visitor-sat", label: "Visitor Satisfaction", value: this.visitorSatisfaction(), category: "satisfaction" as KpiCategory, unit: "%", tooltip: "Fan experience satisfaction score" },
      { id: "risk-score", label: "Risk Score", value: this.executiveRiskScore(70, 75, 65), category: "risk" as KpiCategory, unit: "%", tooltip: "Executive risk assessment score" },
    ];
